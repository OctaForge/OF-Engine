...
plugins = {
    area_trigger = {
        ...
        client_on_collision = function(self, collider)
            ...
            if collider.world_sequences[self:get_sequence_id()] == (self:get_sequence_num() - 1) then
                ...
                if #table.filter(area_triggers, function(i, entity)
                    return (entity:get_sequence_id()  == self:get_sequence_id()  and
                            entity:get_sequence_num() == self:get_sequence_num() and
                            entity:get_sequence_is_mandatory() and
                ...
                for k, entity in pairs(table.filter(area_triggers, function(i, entity)
                    return (entity_get_sequence_id()  == self:get_sequence_id() and
                            entity:get_sequence_num() == self:get_sequence_num()
                    )
                end)) do
                    entity.sequence_is_mandatory_passed = false
                end

                collider.world_sequences[self:get_sequence_id()] = collider.world_sequences[self:get_sequence_id()] + 1
