...
float cursorx = 0.5f, cursory = 0.5f; // INTENSITY: removed 'static'
...
struct gui : g3d_gui
{
    ...
    void icon_(Texture *t, bool overlaid, int x, int y, int size, bool hit)
    {
        ...
        if(overlaid)
        {
            if(!overlaytex) overlaytex = textureload("data/textures/ui/guioverlay.png", 3);
    ...
    {        
        if(visible())
        {
            if(!slidertex) slidertex = textureload("data/textures/ui/guislider.png", 3);
            glEnable(GL_TEXTURE_2D);
    ...
    int button_(const char *text, int color, const char *icon, bool clickable, bool center)
    {
        ...
        if(visible())
        {
            ...
            if(icon)
            {
                defformatstring(tname)("data/textures/icons/%s.jpg", icon);
    ...
    {
        if(!skintex) skintex = textureload("data/textures/ui/guiskin.png", 3);
...
{
    if(guis2d.length() || !GuiControl::isMouselooking()) { x = cursorx; y = cursory; } // INTENSITY: Added mouselooking
    else x = y = 0.5f;
}
