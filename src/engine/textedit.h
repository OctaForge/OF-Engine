...
struct editor 
{
    ...
    void draw(int x, int y, int color, bool hit)
    {
        ...
        for(int i = scrolly; i < lines.length(); i++)
        {
            ...
            if (!password) // INTENSITY: Check for password fields
                draw_text(lines[i].text, x, y+h, color>>16, (color>>8)&0xFF, color&0xFF, 0xFF, hit&&(cy==i)?cx:-1, maxwidth);
            else
            {   // INTENSITY: Added this code to render dummy text in password fields
                int len = strlen(lines[i].text);
                char* dummy_text = new char[len+1];
                for (int j = 0; j < len; j++)
                    dummy_text[j] = '*';
                dummy_text[len] = '\0';
                draw_text(dummy_text, x, y+h, color>>16, (color>>8)&0xFF, color&0xFF, 0xFF, hit&&(cy==i)?cx:-1, maxwidth);
                delete[] dummy_text;
                // INTENSITY: End
            }
