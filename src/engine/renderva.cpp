...
static inline void rendermapmodel(extentity &e)
{
    ...
    rendermapmodel(entity, animval(anim, aflags), e.o, e.attr[0], e.attr[1], e.attr[2], MDL_CULL_VFC | MDL_CULL_DIST, basetime, e.attr[3] > 0 ? e.attr[3]/100.0f : 1.0f); // OF
}
...
static void genshadowmeshmapmodels(shadowmesh &m, int sides, shadowdrawinfo draws[6])
{
    ...
    for(octaentities *oe = shadowmms; oe; oe = oe->rnext) loopvj(oe->mapmodels)
    {
        ...
        if(e.attr[2]) orient.rotate_around_y(sincosmod360(-e.attr[2]));
        if(e.attr[3] > 0) orient.scale(e.attr[3]/100.0f);
