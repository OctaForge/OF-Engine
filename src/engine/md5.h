...
struct md5 : skelmodel, skelloader<md5>
{
    ...
    bool loaddefaultparts()
    {
        ...
        fname++;
        defformatstring(meshname, "media/model/%s/%s.md5mesh", loadname, fname);
        mdl.meshes = sharemeshes(path(meshname), NULL, 2.0);
        if(!mdl.meshes) return false;
        mdl.initanimparts();
        mdl.initskins();
        defformatstring(animname, "media/model/%s/%s.md5anim", loadname, fname);
    ...
    bool load()
    {
        ...
        defformatstring(cfgname, "media/model/%s/md5.cfg", loadname);
