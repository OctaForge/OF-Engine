...
bool visibleface(cube &c, int orient, int x, int y, int z, int size, uchar mat, uchar nmat, uchar matmask)
{
    ...
    if(nsize > size || (nsize == size && !o.children))
    {
        ...
        if(isempty(c) || notouchingface(o, opp)) return true;
...
int visibletris(cube &c, int orient, int x, int y, int z, int size)
{
    ...
    int concave = (e3 = v[3]).sub(v[0]).dot(n);
    if(!concave)
    {
        if(ivec().cross(e2, e3).iszero()) { if(n.iszero()) return 0; vis = 1; touching = 0xF&~(1<<3); }
    ...
    int order = concave > 0 ? 1 : 0, notouch = notouchmasks[order][touching];
...
bool genmergedverts(cube &cu, int orient, const ivec &co, int size, const mergeinfo &m, vec *vv, int vis, plane *p)
{
    ivec e[4], n;
    genfaceverts(cu, orient, e);
    n.cross(e[1].sub(e[0]), e[2].sub(e[0]));
    ivec fo = ivec(co).mask(0xFFF).shl(3);
    int offset = -n.dot(e[0].mul(size).add(fo));

    int dim = dimension(orient), c = C[dim], r = R[dim];
    bool outside = m.u1 < fo[c] || m.u2 > fo[c] + (size<<3) || m.v1 < fo[r] || m.v2 > fo[r] + (size<<3);
    vec vo = ivec(co).mask(~0xFFF).tovec();
    loopi(4)
    {
        const ivec &coords = facecoords[orient][i];
