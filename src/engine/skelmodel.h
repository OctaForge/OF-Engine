...
struct skelmodel : animmodel
{
    ...
    struct skeleton
    {
        ...
        int availgpubones() const { return min(maxvsuniforms, maxskelanimdata) / (matskel ? 3 : 2); }
        ...
        #define INTERPBONES(outbody, rotbody) \
            ...
            { \
                ...
                if(as->cur.anim.flags&ANIMFLAG_NOPITCH || (as->interp < 1 && as->prev.anim.flags&ANIMFLAG_NOPITCH)) \
                    angle *= (as->cur.anim.flags&ANIMFLAG_NOPITCH ? 0 : as->interp) + (as->interp < 1 && as->prev.anim.flags&ANIMFLAG_NOPITCH ? 0 : 1-as->interp); \
                rotbody; \
            }
    ...
    struct skelmeshgroup : meshgroup
    {
        ...
        void intersect(const animstate *as, float pitch, const vec &axis, const vec &forward, dynent *d, part *p, const vec &o, const vec &ray)
        {
            ...
            skelcacheentry &sc = skel->checkskelcache(p, as, pitch, axis, forward, as->cur.anim.flags&ANIMFLAG_RAGDOLL || !d || !d->ragdoll || d->ragdoll->skel != skel->ragdoll ? NULL : d->ragdoll);
        ...
        void render(const animstate *as, float pitch, const vec &axis, const vec &forward, dynent *d, part *p)
        {
            ...
            if(!skel->numframes)
            {
                if(!(as->cur.anim.flags&ANIMFLAG_NORENDER))
                {
                    if(!vbocache->vbuf) genvbo(norms, tangents, *vbocache);
            ...
            if(as->cur.anim.flags&ANIMFLAG_RAGDOLL && skel->ragdoll && !d->ragdoll)
            {
