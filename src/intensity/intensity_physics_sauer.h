...
public:
    ...
    virtual void setBodyAngularVelocity(physicsHandle handle, const vec& angularVelocity) { assert(0); };
    ...
    virtual void getBodyAngularVelocity(physicsHandle handle, vec& angularVelocity) { assert(0); };
    ...
    virtual void rayCastClosest(vec &from, vec &to, float& hitDist, CLogicEntity *&hitEntity, CLogicEntity *ignore = NULL);
};
